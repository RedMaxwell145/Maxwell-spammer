...
def banner():
    ...
    print(r"")
    print()
    print(r" Софт создан Максвеллом, если вы не из группировки salvage squad или Империи Анти-лгбт то идите лесом")
    print(RESET_ALL)
...
def main():
    ...
    print("[3] ВЫХОД.")
    print()
    number = input(f"{BRIGHT}{RED}Введите номер пункта: {RESET_ALL}")
    if number == "1":
        spam_handler()
    elif number == "2":
        update()
        ...
        print(f"\n{BRIGHT}{RED}[*] Животное, введи правельный пункт!{RESET_ALL}")
        sleep(1)
        main()
...
def spam_handler():
    ...
    check_version()
    banner()
    print("Введите номер телефона говнюка")
    phone = parse_phone(input(f"{BRIGHT}{RED}spammer >> {RESET_ALL}"))
    print()
    print("Использовать прокси для анонимности? (y/n)")
    proxy = input(f"{BRIGHT}{RED}spammer >> {RESET_ALL}")
    if proxy.lower() == "y":
        proxies = generate_proxy()
    else:
        proxies = None
    print()
    print("Введите кол-во СПАМОВ")
    threads = input(f"{BRIGHT}{RED}spammer >> {RESET_ALL}")
    if threads in ["", " ", "0"]:
        threads = "1"
    try:
        if int(threads) > 10:
        ...
        threads = "1"
    banner()
    print(f"Телефон лоха: {BRIGHT}{GREEN}{phone}{RESET_ALL}")
    print("Спамер запущен, хэхэ", end="\n")
...
def check_internet():
    try:
        ...
        print(f"\n{BRIGHT}{GREEN}[*] No connection avaible please idite nahui!{RESET_ALL}")
        sleep(1)
        main()
    return
...
def check_version():
    ...
    if float(version) < float(get("https://raw.githubusercontent.com/cludeex/spammer/master/version.txt").text):
        print(f"\n{BRIGHT}{RED}[*] Аутист версия устарела!{RESET_ALL}")
        sleep(1)
        main()
    return
...
def update():
    check_internet()
    banner()
    print("Вы уверены что хотите жеж? (y/n)")
